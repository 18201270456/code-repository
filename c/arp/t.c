...
struct in_addr get_src_ip(char *devices)
{
    ...
    {
        if (setsockopt
            (sock_id, SOL_SOCKET, SO_BINDTODEVICE, device,
             strlen(device) + 1) == -1)
...
int
create_pkt(unsigned char *buf, struct in_addr src, struct in_addr dst,
           struct sockaddr_ll *FROM, struct sockaddr_ll *TO)
{
...
void send_pkt()
{
    ...
    int cc =
        sendto(socket_id, send_buf, pkt_size, 0, (struct sockaddr *) &he,
               sizeof(he));
    if (cc == pkt_size)
        send_count++;
    alarm(1);
...
int chk_recv_pkt(unsigned char *buf, struct sockaddr_ll *FROM)
{
    ...
    if (ah->ar_op != htons(ARPOP_REQUEST)
        ...
        return 0;
    if (ah->ar_pro != htons(ETH_P_IP) || ah->ar_pln != 4
...
void disp_info(int received, struct in_addr dst, int msecs, int usecs,
               struct sockaddr_ll from)
{
    ...
    printf(" [%02X:%02X:%02X:%02X:%02X:%02X] ", from.sll_addr[0],
           from.sll_addr[1], from.sll_addr[2], from.sll_addr[3],
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        int recv_size =
            recvfrom(socket_id, recv_buf, sizeof(recv_buf), 0,
        ...
        if (chk_recv_pkt(recv_buf, &from) > 0)
        {
            ...
            {
                usecs =
                    (recv_time.tv_sec - send_time.tv_sec) * 1000000 +
