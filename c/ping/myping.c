...
static ping_packet pingpacket[128];
...
static void icmp_pack(struct icmp *icmph, int seq, struct timeval *tv,
                      int length);
...
{
    int sum = 0;                /* 计算结果 */
    int odd = len & 0x01;       /* 是否为奇数 odd=1 or 0; */
            ...
            default:
                {
                    ...
                    /* 接收数据 */
                    int size =
                        recv(rawsock, recv_buff, sizeof(recv_buff), 0);
                    if (errno == EINTR)
                    {
